<template>
   <div id="home_app" v-cloak>
      ...
      <div class="home-content" v-for="(contentItem, index) in contentData">
				...
				<div class="home-content-title"><i></i><img :src="contentIcon[index].img" 
        :style="{'width': contentIcon[index].width + 'rem'}"/>
        <span class="font-size-32">{{contentItem.title}}</span><i></i></div>
				<div :class="{'home-content-bottom': sdx == 2 || sdx == contentItem.bookListArray_sub.length-1}" v-for="(sortItem, sdx) in contentItem.bookListArray_sub">
				    <img class="home-main-add" style="margin-bottom: .888rem;" 
            src="/static/img/home/index-ad-13.jpg" v-if="index == 0 && sdx == 3" />
					<div class="home-content-main">
						<div class="home-content-sort">{{sortItem.name}}<span class="font-size-24"
             @click="gotoBooklist(sortItem.path)">更多书单</span></div>
					    <ul>
